// @Description Rewritten_Sub_804A244_Tmain_doramori_dorizame
// @HookAddress 0x78EB80
// @HookString P
// Made by beco
// Verison 1.0
...
#define Sub_804A244_Tmain_doramori_dorizame ((void (*)()) 0x804A245)
...
void Rewritten_Sub_804A244_Tmain_doramori_dorizame() {
    // Vanilla code
    Sub_804A244_Tmain_doramori_dorizame();

    // Custom code
    if (CurrentEnemyData.CurEnemy_TWork2 != 0x08){
        if (CatbatCuckooHp <= 6) {
            CurrentEnemyData.CurEnemy_paletteId = 0x08;
            CurrentEnemyData.CurEnemy_TWork2 = 12; // Set shark bot speed (Default:0x01)
        } else if (CatbatCuckooHp <= 9) {
            CurrentEnemyData.CurEnemy_paletteId = 0x01;
            CurrentEnemyData.CurEnemy_TWork2 = 6; // Set shark bot speed (Default:0x01)
        } else {
            CurrentEnemyData.CurEnemy_TWork2 = 3; // Set shark bot speed (Default:0x01)
        }
    }
    // Damage functions
    if(CurrentEnemyData.CurEnemy_TWork3 != CatbatCuckooHp) {
        if (CatbatCuckooHp == 6) {
            Sub_80766E8_TOptObjSet(CurrentEnemyData.CurEnemy_YPos, CurrentEnemyData.CurEnemy_XPos, 0x07); // Generate splash
        } else if (CatbatCuckooHp == 9) {
            Sub_80766E8_TOptObjSet(CurrentEnemyData.CurEnemy_YPos, CurrentEnemyData.CurEnemy_XPos, 0x07); // Generate splash
        }
    }
    CurrentEnemyData.CurEnemy_TWork3 = CatbatCuckooHp;
}
