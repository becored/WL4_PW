// @Description Rewritten_Sub_8056028_EntityAI_0x69_Tmain_boss_shittuki_puutin
// @HookAddress 0x78E9A4
// @HookString P
// Made by beco
// Verison 1.0
...
#define Sub_8056028_EntityAI_0x69_Tmain_boss_shittuki_puutin ((void (*)()) 0x8056029)
#define EntityCollision ((int (*)(int, int)) 0x8026211)

#define sin_cos_table ((signed short*) 0x8095138)

#define AerodentHp (*(volatile unsigned char*) 0x3000179) // Aerodent's health
#define BossHpTemp (*(volatile unsigned char*) 0xE000E21)

void Rewritten_Sub_8056028_EntityAI_0x69_Tmain_boss_shittuki_puutin() {
    // Vanilla code
    Sub_8056028_EntityAI_0x69_Tmain_boss_shittuki_puutin();

    // Custom code
    // Damage functions
    if (BossHpTemp != AerodentHp) {
        CurrentEnemyData.CurEnemy_padding ++;
    }
    BossHpTemp = AerodentHp;
    switch (CurrentEnemyData.CurEnemy_CurrentAnimationId) {
        case K0_WALK:
        // When Aerodent is upside down
            if (CurrentEnemyData.CurEnemy_TWork1 == 8) {
                for (int c = 0 ; c < 24 ; c ++) {
                    if (ENTITIES[c].CurEnemy_GlobalId == BOSS_SHITTUKI) {
                        if (CurrentEnemyData.CurEnemy_usStatus & 0x1000) {
                            CurrentEnemyData.CurEnemy_XPos += CurrentEnemyData.CurEnemy_padding;
                            ENTITIES[c].CurEnemy_XPos += CurrentEnemyData.CurEnemy_padding;
                        } else {
                            CurrentEnemyData.CurEnemy_XPos -= CurrentEnemyData.CurEnemy_padding;
                            ENTITIES[c].CurEnemy_XPos -= CurrentEnemyData.CurEnemy_padding;
                        }
