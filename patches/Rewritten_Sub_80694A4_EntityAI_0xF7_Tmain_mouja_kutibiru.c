// @Description Rewritten_Sub_80694A4_EntityAI_0xF7_Tmain_mouja_kutibiru
// @HookAddress 0x78EBDC
// @HookString P
// Made by beco
// Verison 1.0
...
#define Sub_80694A4_EntityAI_0xF7_Tmain_mouja_kutibiru ((void (*)()) 0x80694A5)
#define EntityCollision ((int (*)(int, int)) 0x8026211)

#define sin_cos_table ((signed short*) 0x8095138)

void Rewritten_Sub_80694A4_EntityAI_0xF7_Tmain_mouja_kutibiru() {
    // Vanilla code
    Sub_80694A4_EntityAI_0xF7_Tmain_mouja_kutibiru();

    // Custom code
    switch (CurrentEnemyData.CurEnemy_CurrentAnimationId) {
        case Q_JITTO:
            CurrentEnemyData.CurEnemy_padding = 120;
            break;
        case K0_WALK:
        case K1_LEFT:
        case K1_RIGHT:
            CurrentEnemyData.CurEnemy_padding --;
            if (CurrentEnemyData.CurEnemy_padding <= 0) {
                CurrentEnemyData.CurEnemy_padding = 120;
            } else if (CurrentEnemyData.CurEnemy_padding <= 40) {
                CurrentEnemyData.CurEnemy_GuardAndDamageParam = NO_TOGE;
                CurrentEnemyData.CurEnemy_paletteId = 0x0;
            } else {
                CurrentEnemyData.CurEnemy_GuardAndDamageParam = ALL_TOGE;
                if (CurrentEnemyData.CurEnemy_usStatus & 0x40) {
                    CurrentEnemyData.CurEnemy_XPos += 6;
                } else {
                    CurrentEnemyData.CurEnemy_XPos -= 6;
                }
                if ( ucMainTimer & 2 ) {
                    CurrentEnemyData.CurEnemy_paletteId = 0x4;
                } else {
                    CurrentEnemyData.CurEnemy_paletteId = 0x0;
                }
            }
